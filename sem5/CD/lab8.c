...
void currentlevel(struct treeNode *root, int level, int h);
int height(struct treeNode *node);

void levelorder(struct treeNode *root) // Function to print level order traversal a tree
{
    int h = height(root);
    int i;
    for (i = 1; i <= h; i++)
    {
        for (int j = 0; j < pow(2, h - 3) - i; j++)
            printf("  ");
        currentlevel(root, i, h - i);
        printf("\n\n");
    }
}

void currentlevel(struct treeNode *root, int level, int h) // Print nodes at a current level
{
    if (root == NULL)
    {
        return;
    ...
    {
        currentlevel(root->left, level - 1, h);
        printf("  ");
        currentlevel(root->right, level - 1, h);
    }
}

int height(struct treeNode *node) // Compute the "height" of a tree -- the number of nodes along the longest path from the root node down to the farthest leaf node.
{
    if (node == NULL)
        return 0;
    else
