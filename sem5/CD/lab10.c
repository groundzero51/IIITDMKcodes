...
struct token // map structures to maintain count of unique tokens
{
    int count;
    char token[100];
};
...
void increment_token(char *token) // to increment the count of token
{
    bool flag = true;
...
bool isDelimiter(char ch) // Returns true if the character is a DELIMITER.
{
    if (ch == ' ' || ch == '+' || ch == '-' || ch == '*' ||
        ch == '/' || ch == ',' || ch == ';' || ch == '>' ||
        ch == '<' || ch == '=' || ch == '(' || ch == ')' ||
...
bool isSpecialSymbol(char ch)       // Returns true if the character is a SPECIAL SYMBOL.
{
...
void start(char *filename)      // Function for removing redundant lines, spaces and comments
{
    FILE *reqFilePtr = fopen(filename, "r");
    FILE *outputFilePtr = fopen("temp.c", "w+"); // Output file
    int prevChar = '\n';
    int currentChar = fgetc(reqFilePtr);
    while (currentChar != EOF)      //removes redundant lines, spaces, comments and preprocessor directives
    {
