...
int main()
{
    ...
    printf("Enter OTP to send when phone number is received : ");
    fgets(OTP,100,stdin);
    ...
    {
        printf("\nNo errors in bind. Enter phone number on client\n");
        listen(s_socket, 5);
        socklen_t add;
        add = sizeof(other);
        s_server = accept(s_socket, (struct sockaddr *)&other, &add);
        sendto(s_socket,OTP,sizeof(OTP),0,(struct sockaddr*)&server,sizeof(server));
        recvfrom(s_socket,OTP,sizeof(OTP),0,(struct sockaddr*)&other,&add);
